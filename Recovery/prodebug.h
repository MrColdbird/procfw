...
/** 
  ...
  * @note To reset the entire screens bg colour you need to call proDebugScreenClear
  *
  * @param color - A 32bit RGB colour
  */
void proDebugScreenSetBackColor(u32 color);
...
/**
  ...
  */
void proDebugScreenSetTextColor(u32 color);
...
/** 
  ...
  */
void proDebugScreenPutChar(int x, int y, u32 color, u8 ch);
...
*/
int proDebugGetStackTrace(unsigned int* results, int max);
...
typedef struct _proDebugRegBlock
{
	u32 frame[6];
...
typedef void (*proDebugErrorHandler)(proDebugRegBlock *regs);
...
/** 
  ...
  * @return < 0 on error
  */
int proDebugInstallErrorHandler(proDebugErrorHandler handler);

/**
  * Dump an exception to screen using the proDebugScreen functions.
  ...
  *
  */
void proDebugDumpException(proDebugRegBlock *regs);

/** Type for Kprintf handler */
typedef int (*proDebugKprintfHandler)(const char *format, u32 *args);
...
/** 
  ...
  * @return < 0 on error.
  */
int proDebugInstallKprintfHandler(proDebugKprintfHandler handler);

/** Structure to hold a single stack trace entry */
typedef struct _proDebugStackTrace
{
...
/**
  ...
  * @param trace - Pointer to an array of proDebugStackTrace structures.
  * @param max - The maximum number of traces to make.
  *
  * @return The number of functions found.
  */
int proDebugGetStackTrace2(proDebugRegBlock *regs, proDebugStackTrace *trace, int max);

/** Structure to hold the psp profiler register values */
typedef struct _proDebugProfilerRegs
{
	volatile u32 enable;
...
} proDebugProfilerRegs;

/** Enables the profiler hardware */
void proDebugProfilerEnable(void);

/** Disables the profiler hardware */
void proDebugProfilerDisable(void);

/** Clear the profiler registers */
void proDebugProfilerClear(void);

/** Get the profiler register state
  *
  * @param regs - A pointer to a proDebugProfilerRegs structure.
  */
void proDebugProfilerGetRegs(proDebugProfilerRegs *regs);

/** Print the profiler registers to screen */
void proDebugProfilerPrint(void);

/** Type for the debug print handlers */
typedef int (*proDebugPrintHandler)(const char *data, int len);

/** Type for the debug input handler */
typedef int (*proDebugInputHandler)(char *data, int len);
...
/**
  ...
  */
int proDebugInstallStdinHandler(proDebugInputHandler handler);
...
/**
  ...
  * @return < 0 on error, else 0.
  */
int proDebugInstallStdoutHandler(proDebugPrintHandler handler);
...
/**
  ...
  * @return < 0 on error, else 0.
  */
int proDebugInstallStderrHandler(proDebugPrintHandler handler);
 ...
 */
void proDebugSioPutchar(int ch);
 ...
 */
void proDebugSioPuts(const char *str);
 ...
 */
int proDebugSioPutData(const char *data, int len);
